	...
	while(1)
	{
		...
		if(forkResult == 0)
		{
			...
			while(1)
			{
				...
				{
					//read the part of the input that specifies the row selection.
					char rowArgument[10];
					...
					//adjust the target row properly for the array.
					targetRow--;
					//If the row exists...
					if(targetRow < sizeof(movies)/sizeof(movie_t) && targetRow >= 0)
					{
						//...compose the fields of this row into a message. Brute force!
						...
						strcat(sendMessage, ", ");
						char temp[12];
						sprintf(temp, "%hi", movies[targetRow].year);  //convert short into string before appending.
						strcat(sendMessage, temp);
						strcat(sendMessage, ", ");
						sprintf(temp, "%hi", movies[targetRow].oscars_won); //convert short into string before appending.
						strcat(sendMessage, temp);
						strcat(sendMessage, ", ");
						strcat(sendMessage, movies[targetRow].origin_country);
						strcat(sendMessage, "\n");
						
						//send the composed message back to the client.
		...
		//GO BACK TO STEP 3 if Parent, closing Child OR Fork failiure
